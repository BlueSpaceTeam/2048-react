 ...
 * @LastEditTime: 2022-06-30 16:55:34
 * @LastEditors: tim.wen
 * @Description:
 * @FilePath: /2048-react/react/src/components/Game/Board.tsx
 */
...
const Board: React.FC<IPropBoard> = (props) => {
  ...
  const SquaresUI: JSX.Element[] = squares.map(
    (n: number, idx: React.Key | null | undefined) => {
  ...
  function getDirection(
    ...
    if (deltaDir > 0) {
      // 如果deltaX - deltaY > 0 ,认为 往→ 或 ←
      direction = dirX;
    } else if (deltaDir < 0) {
      // 如果deltaX - deltaY < 0 ,认为 往↓ 或 ↑
      direction = dirY;
    } else {
      // 如果deltaX - deltaY = 0, 随机上下 或 左右
  ...
  function handleTouchEnd(e: React.TouchEvent) {
    ...
    const direction: Direction | null = getDirection(
